...
class NeuralNetwork:
    ...
    def query_img(self, img_array):
        # 隐含层的输入参数=w_hidden_output × inputs
        hidden_inputs = numpy.dot(self.w_input_hidden, img_array)
        # 计算从隐含层的激活函数值
        hidden_outputs = self.activation_function(hidden_inputs)
        # 计算隐含层到输出层的输入
        final_inputs = numpy.dot(self.w_hidden_output, hidden_outputs)
    ...
    def load(self, w_input_hidden_file, w_hidden_output_file):
        self.w_input_hidden = numpy.loadtxt(w_input_hidden_file)
        self.w_hidden_output = numpy.loadtxt(w_hidden_output_file)
